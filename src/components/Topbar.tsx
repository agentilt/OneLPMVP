      ...
      <div className="relative h-18 sm:h-20 px-4 sm:px-6 lg:px-10 flex items-center justify-between gap-4 glass-panel-soft border-b border-border shadow-2xl shadow-black/10">
        <div className="flex items-center gap-3 min-w-0">
          {onMenuClick && (
            <button
              onClick={onMenuClick}
              className="lg:hidden p-2 rounded-2xl bg-[var(--surface)]/90 border border-border/70 hover:border-accent/50 transition-all duration-200 shadow-sm hover:shadow-lg hover:scale-[1.01] active:scale-[0.99]"
          ...
          <button
            onClick={openSearch}
            className="group flex-1 inline-flex items-center gap-3 px-4 py-2.5 rounded-2xl glass-panel border border-border shadow-lg shadow-black/10 hover:border-accent/50 hover:shadow-accent/20 transition-all backdrop-blur text-left"
            aria-label="Open global search (⌘K)"
          >
            <div className="w-9 h-9 rounded-xl glass-panel bg-[radial-gradient(circle_at_35%_35%,rgba(107,220,255,0.18),transparent_55%),linear-gradient(135deg,#7c5bff,#6bdcff)] flex items-center justify-center ring-1 ring-white/10">
        ...
        <div className="flex items-center gap-2">
          <div className="md:hidden">
            <button
              onClick={openSearch}
              className="p-2 rounded-xl glass-panel border border-border/70 shadow-sm hover:shadow-md transition"
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 pl-2 pr-3 py-2 rounded-full glass-panel border border-border hover:border-accent/60 transition-all duration-150 shadow-sm hover:shadow-md backdrop-blur text-foreground"
            >
            ...
            {showUserMenu && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-72 glass-panel backdrop-blur-xl border border-border rounded-2xl shadow-[0_30px_90px_rgba(5,10,30,0.55)] z-50 py-2 overflow-hidden">
